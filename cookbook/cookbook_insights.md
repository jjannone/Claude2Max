...
> *Why it matters*: any Claude2Max patch with a user-facing volume control should default to `gain~` or to the dB-mapped slider+`sel`+`dbtoa` pattern. Don't ship a linear slider mapped directly to `*~`. Belongs in `patching/MAX_PATCHING.md` UI section.
...
> *Why it matters*: any Claude2Max stereo panner should default to constant-power; the implementation cost is two `sqrt~` objects vs none. Belongs in `patching/MAX_PATCHING.md` audio-routing patterns section.
...
> *Why it matters*: every `dac~` in a Claude2Max teaching/example patch should be initialized via `startwindow`, not `start`. Belongs in `patching/MAX_PATCHING.md` "Common Pitfalls" section.
...
> *Why it matters*: any Claude2Max synth voice spec should plan from the start for parallel ADSRs sharing a gate, not a single envelope reused for both targets. Belongs in `patching/MAX_PATCHING.md` synthesis-patterns section.
...
> *Why it matters*: any Claude2Max patch that wants a soft-blur effect on video should use this idiom rather than rolling a `jit.fastblur` chain or a custom shader. Belongs in `patching/MAX_PATCHING.md` video-patterns section.
...
> *Why it matters*: when a Claude2Max patch needs to store more than a few dozen values (sequence state, particle positions, sample-and-hold buffers, lookup tables), `jit.matrix` is more memory-efficient and faster to access than `coll`/`zl`/JS arrays. Belongs in `patching/MAX_PATCHING.md` data-storage patterns section. The mental shift "matrix = arbitrary multi-dim array" unlocks a lot of patterns.
