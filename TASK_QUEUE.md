...
- [pending] **Low priority. `RefpageCache._find_xml`: find refpages one folder deeper under `docs/` and `docs/refpages1/`** — Added 2026-09-14 after `RefpageCache` learned to search user packages (`~/Documents/Max 9/Packages`). Inside each package folder, `_find_xml` in `spec2maxpat.py` checks `docs/`, `docs/refpages/`, `docs/refpages1/` and one domain folder under `docs/refpages/`. Real layouts it still misses, measured on John's machine that day: built-in packages keep 83 pages in `docs/jit.fx/` and 5 in `docs/refpages1/<domain>/` (88 in all); the user package Data Knot keeps 129 in `docs/dataknot-ref/`. Those objects currently read as "no refpage", so the gate cannot resolve their inlets and outlets and does not check their attributes or messages. Deliverable: in the per-package loop of `_find_xml`, also check `docs/<dir>/<name>.maxref.xml` for every subfolder of `docs/` and `docs/refpages1/<dir>/<name>.maxref.xml`, keeping the current order (flat folders first, built-in packages before user packages). **Why this is not a free fix:** objects that gain a refpage start being checked by `attribute-resolves` and `message-unverified`, so the verifier may begin reporting on patches that currently read clean. Before landing, sweep `patches/` and the Butter_tools patches with `verify_patch` before and after, and list any new findings for John. Tests: extend `tests/test_user_package_refpages.py` with a `docs/somepkg-ref/` page and a `docs/refpages1/common/` page, both in temporary folders. Update the subfolder list in `patching/MAX_PATCHING.md` > *Shipping a Community Max Package*. Model: Sonnet.
