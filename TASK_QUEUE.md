...
- [in progress] **Forum Knowledge Crawl** — Systematically crawl the Cycling '74 forums in chunks, extracting Max principles, techniques, and approaches from experienced community members. Build a growing reference of non-obvious patching knowledge that supplements the official docs.
  ...
  *In progress (2026-05-05c — pass-4 chunk done): **1177 threads total in state** (933 pre-pass-4 + 248 added in pass-4 — split 110 artifact-bearing / 138 zero-artifact). Pass-4 covered Gen pp.9-15 + Jitter pp.19-22 + MaxMSP pp.21-24 + Javascript pp.16-19 + RNBO pp.11-12 + Max For Live pp.13-15 + Misc pp.11-12. 72 artifact files saved to `/tmp/forum-pass4/`. 14 insights written across 6 sections of `c74-forum/forum_insights.md` from selective WebFetch on the top 16 artifact-richest threads (94% hit rate). 5 PROMOTION-CANDIDATEs were promoted to reference docs in the same pass (closing-sweep cadence — see CLAUDE.md). State decisions: 522 downloaded / 649 skipped-no-artifact / 6 phase-C-scraped. **Architectural finding holds**: Gen subforum remains the densest — 6 of 14 pass-4 insights came from Gen alone.*

  **Resume point**: pick the next subforum chunk per `c74-forum/FORUM_CRAWL_LOG.md`. Candidate areas now: Gen pp.16+ (still highest priority), Jitter pp.23+, MaxMSP pp.25+, Javascript pp.20+, RNBO pp.13+, Max For Live pp.16+, Misc pp.13+. Java pp.6+ remains deprioritised. Use `enumerate_forum_threads.py --diff` against `c74-forum/forum_crawl_state.json` to surface only NEW or UPDATED threads. Run from local laptop (or any non-Cloudflare-WAF egress); the Anthropic cloud env is blocked.
