...
- [in progress] **Claude2Max MCP server + global enforcement layer — knowledge as queryable tools, Max-file edits gated until tools fire** — Build a Model Context Protocol server (`claude2max-mcp`) that exposes Max-patching knowledge as first-class callable tools, paired with a global skill + PreToolUse hook that mechanically ensures Claude has consulted the tools before editing any Max file. Ship all three pieces designed together as one architecture: MCP for *knowledge representation*, skill for *active invocation*, hook for *enforcement*. The architecture replaces today's prose-pointer-and-hope-Claude-reads-it pattern with knowledge-as-tools-and-can't-act-without-them.
  ...
  *In progress (2026-06-13): **Phase (i) Step 2 complete** (server skeleton + `essentials()` tool). Deliverables: `mcp_server/server.py` (FastMCP, one tool `essentials()` returning ~1.4K-token structured binding-rule summary), `mcp_server/__init__.py`, `mcp_server/README.md` (venv install + `claude mcp add` instructions + tool surface table), `.gitignore` updated (`mcp_server/.venv/`). Venv created at `mcp_server/.venv/` with `mcp>=1.27.0` installed (Homebrew Python enforces PEP 668, requires venv). Server verified: FastMCP instantiates, `essentials` tool registers, `essentials()` returns 6937 chars / 1065 words. Registered user-scope: `claude mcp add --scope user claude2max -- <venv>/python3 <repo>/mcp_server/server.py` → written to `~/.claude.json`. Smoke test requires a fresh Claude session (not executable from within this session). **Resume at Step 3** (`lookup_object()` — wraps `RefpageCache` from `spec2maxpat.py` + `packages/package_objects.json`). Platform note: macOS with Homebrew Python requires venv for `mcp` install (PEP 668); README and installer (Step iii) must document this. Step 3 is implementation; can be done on Sonnet.*
