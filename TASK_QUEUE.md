...
- [in progress] **Fix `butter_keymap.amxd`: confirm per-instance persistence of the deleted set, then re-test the whole device after the butter_keys changes** — Added 2026-09-14 at John's request. The device lives in the Butter_tools package (`/Users/johnjannone/Library/CloudStorage/Dropbox-JohnJannone/john jannone/_Projects__________________/_GitHub/Butter_tools`, its own private git repo, symlinked into `~/Documents/Max 9/Packages/`): `devices/butter_keymap.amxd` is the Max MIDI Effect, `javascript/keymap.js` its engine, `javascript/zslider.js` the two keyboards. Read the package `README.md` (butter_keymap and zslider sections) and `patching/M4L_PATCHING.md` > *Per-instance state that must survive the Live set* before starting.
  ...
  **Open problems.**
  ...
  2. **The device has not been opened in Live since the zslider rework** (zkeyboard renamed zslider, `z_` attribute names, four outlets instead of three, `@offset` now the exact lowest note, `@mode` gains `z_mono`). The rename session rewired both boxes but did not run the device.
  3. **Both zslider boxes carry a Max-saved `embedstate` with the pre-rename names** (`clicknotes`, `keyaspect`, `slidermode`, `dotsize`, …). zslider still accepts those as legacy messages. Whether v8ui's restore reaches those functions or posts console errors is unverified. The engine's `init` re-sends what the device depends on (`mode 1`, `z_clicknotes 0`, `z_keyaspect 6`, `z_lohikey 0 127`), so this is a console check, not a blocker. A save from Max should rewrite the block with `z_` names; confirm it does.
  ...
  **Work, in order.**
  (a) **Ask John to run the test.** Load the device from `Butter_tools/devices/` on two MIDI tracks. Delete different keys in each and confirm each shows only its own tints and readout while running. Save the set, close it, reopen. Report each instance's readout and tints, and any Max Console line naming zslider or keymap.
  (b) **If the two instances still interfere while running**, search the device, `keymap.js` and `zslider.js` for any name that is still global: `s` / `r` / `value` / `buffer~` / `dict` / `coll` without `---`, `new Global`, `messnamed`. Rule: `CLAUDE.md` > *Every Global Name Inside a Copied Thing Is Per-Instance*.
  ...
  (e) **Commit only `devices/butter_keymap.amxd` and `javascript/keymap.js`** in Butter_tools. The working tree carries another session's uncommitted zslider work (`README.md`, `help/zslider.maxhelp`, `javascript/zslider.js`, `docs/zslider.maxref.xml`, `examples/attrtest/`, `init/`); leave it out.
