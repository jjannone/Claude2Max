...
- [pending] **Continue the `jjannone-max` corpus crawl past enumeration, and make the MCP server search it** — Added 2026-09-15 at John's request. The crawl indexes John's own `.maxpat` corpus — every patch under `/Users/johnjannone/Library/CloudStorage/Dropbox-JohnJannone/john jannone/_Projects__________________` — as the personal-corpus parallel to `c74-forum/`, `cookbook/` and `c74-projects/`. Nineteen years of patches are the one body of Max knowledge no public source has, and it is the only one that can answer "did John already build this?"

  **Where the work sits.** `jjannone-max/` at the repo root, committed 2026-09-15 at John's direction. It had been sitting in an orphaned worktree folder (`.claude/worktrees/serene-noyce-e53974/`) that git could not see and that is gitignored, so it was in no commit in any branch until then.
