...
- [pending] **Rule enforcement moves into the MCP server — one library, thin callers** — Make `mcp_server/` the single home for every mechanically-checkable and post-hoc-verifiable rule in the repo, and reduce `hooks/` to thin callers into it. Today the rule corpus lives as prose in six `*.md` files and is enforced (partially) by five independent hook scripts plus a gate inside `spec2maxpat.py`. This task ends the split: knowledge and checking logic live in one importable library; every entry point — MCP tool, hook, converter, CLI — calls that library rather than reimplementing it.
  ...
  **Work items**:
  ...
  2. **Promote the existing post-hoc rules from warn to block.** `mcp_server/claude2max_verify/rules.py` already implements 16 rule functions, and 8 of them are exactly the structural invariants this task cares about — `rule_presentation_required`, `rule_presented_controls_need_labels`, `rule_hidden_box_cords_hidden`, `rule_redundant_message_box`, `rule_subpatcher_labels`, `rule_debug_marking`, `rule_allcaps_names`, `rule_prefer_v8`. They run today at `WARNING`/`STYLE`, and `spec2maxpat.py convert` blocks only on `ERROR`, so they are visible but not binding. The work is not writing them; it is (a) running them across `patches/*.maxpat` to measure the false-positive rate, (b) fixing the genuine violations they surface, (c) promoting each clean family to `ERROR`, and (d) deciding which should stay advisory forever (`rule_prefer_v8` is a preference, not an invariant — it should probably remain `STYLE`).

     Two families in this class are genuinely absent and do need writing: **embedded-spec presence** (a `.maxpat` produced via Claude2Max must carry the hidden `text.codebox` with the `--- CLAUDE2MAX SPEC ---` markers) and **spec-vs-patch object-count drift** (the 1669-vs-135 orphan case — compare `len(spec.objects)` to `len(patch.boxes)` and flag divergence beyond a small threshold).
