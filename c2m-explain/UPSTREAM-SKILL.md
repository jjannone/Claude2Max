...
> **This is the upstream/distribution copy of the manifest, generated by `tools/build_upstream_skills.py`.** The authoritative copy is `.claude/skills/c2m-explain/SKILL.md`; edit that one and re-run the script.
...
Pass the patch path as a `.maxpat` path. For the default walkthrough mode:

```bash
python3 c2m-explain/c2m_explain.py <patch.maxpat>
```

For a terser one-line-per-stage view (useful when iterating on a patch and "remind me what this does" is enough):

```bash
python3 c2m-explain/c2m_explain.py <patch.maxpat> --summary
```
...
If invoked without a `.maxpat` path, ask the user which `.maxpat` to explain — don't guess from the working directory.
