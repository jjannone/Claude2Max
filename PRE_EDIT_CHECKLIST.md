...
- If the presentation uses `panel` boxes: does every panel carry `attrs.background: 1`, and does the spec root set `bglocked: 1`? (The converter orders panels last on its own; the verifier warns on the other two.)
