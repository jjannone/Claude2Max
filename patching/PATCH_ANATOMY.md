...
> **Verified 2026-09-15 on a machine with Max 9 installed.** Every object name in
> this document was checked against Max's own registry
> (`interfaces/obj-qlookup.json`, 1,323 entries including aliases), its refpages,
> and `packages/package_objects.json` — 101 of 102 resolve. The one exception is
> `shell`, marked in §8. Package names in §3 were checked the same way, and the
> three the library does not know are marked where they appear. The document was
> first written on a machine without Max, which is why this pass was owed.
...
- **MaxScore** — engraving-quality notation *(not in `packages/package_objects.json`; confirm it is installed before planning around it)*

### Audio analysis / ML

- **FluCoMa** (`fluid.*`) — corpus-based ML, slicing, descriptors
- **MuBu For Max** — multimodal data, gesture, segmentation. The package is `MuBu For Max`; `pipo` is one of its objects, and `IMTr` is not a namespace this library knows — search the library rather than guessing a prefix
...
- **HOA Library** — higher-order ambisonics *(not in `packages/package_objects.json`; confirm it is installed before planning around it)*
- **spat / spat5** — IRCAM spatialization *(not in `packages/package_objects.json`; confirm it is installed before planning around it)*
...
- **`js` / `jsui`** — older SpiderMonkey runtime (ES5-ish). `jsui` draws custom UI to a canvas via the global `sketch` object (22 of Cycling '74's own shipped scripts use it), or `mgraphics` (11), which is the newer of the two.
...
- **IPC / OS** — `shell` (a third-party external, and the one name in this document that could not be verified here: it is neither a Max built-in nor in `packages/package_objects.json`), `serial` for hardware, `hi` for HID devices
