...
- **A `v8` / `js` box that names a script without `@embed 1` dies the moment the `.js` does not travel with the patch.** Max's failure is one console line ("can't find file") and a box that accepts messages and does nothing — no red box, because `v8` itself loaded fine. Write `v8 name.js @embed 1` every time (arguments before the attribute: `v8 name.js 15 @embed 1`); the converter then stores the source inside the box and the verifier warns when the attribute is missing. Keep the `.js` next to the patch for editing and run `sync` after changing it; sync refreshes the stored copy from the file (and restores a missing file from the stored copy). Rule: `CLAUDE.md > Embed the Script in Every v8 Box`.
