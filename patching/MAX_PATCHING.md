...
**Size `@text_width` per attribute-name length** — the default truncates anything over ~5 characters. The label portion of an attrui that's `displaymode 0 @orientation 0` is sized by `text_width` in pixels. At Max's default font size, estimate `len(attr_name) * 8 + 14` pixels (round up) for the label slot; the value widget takes the remaining width of the box. *For instance:* `smoothness` (10 chars) wants `text_width ≈ 94`; `hueMin` (6 chars) wants `text_width ≈ 62`; `visible` (7 chars) wants `text_width ≈ 70`. Without per-name sizing, the screen ends up looking like `smoo… [0.6]` / `min… [0.2]` / `max… [1.5]` — the values are usable but the labels are gibberish.
