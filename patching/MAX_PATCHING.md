...
- Set patcher `width` and `height` large enough to contain all objects without scrolling, including info comments. Leave margin below the lowest object. **It is the view the patch opens in that must fit.** A patch with any presented box opens in presentation (the converter sets `openinpresentation` then), so its window is sized to the presented boxes and its patching view may scroll; a patch with none opens in patching view, and that is what must fit. The verifier's `box-off-canvas` reports one finding per patch with the window size the view actually needs — a WARNING when the operator's presentation is cut off, STYLE for the patching view. *For instance:* on 2026-09-16 every effect shootout opened in a window shorter than its presentation — `reverb-shootout` is 1444 × 852 around a presentation 1183 px tall, so its last 22 boxes sat below the fold.

  **In a tabbed patch the view that opens is a TAB, so the root's own boxes are the wrong thing to measure.** The window is the root patcher's `rect`, and the tabs draw inside it — a root sized to fit its own handful of boxes opens a window too small for every tab in it. Size the root from the tallest tab (plus the tab strip, about 34 px), capped at a height that still opens whole on the smallest screen the patch is for; the one or two longest tabs then scroll, which is what C74 does too — `metro.maxhelp` is root 573 × 510 around tabs of 484, and `butter_keys.maxhelp` root 700 × 560 around tabs up to 814. **The verifier does not catch this**: `box-off-canvas` measures each patcher against its own boxes, and a root whose nine boxes fit its rect is reported clean however tall its tabs are. *For instance:* `butter_comment.maxhelp` opened 207 px tall — the height of its header, tab row and index comment — while its tabs ran from 450 to 1243 px, so every tab showed a header and the top of one demo. (2026-09-22.)
...
<!-- The four sections below came from the Max userguide crawl (2026-09-15), promoted from
     scans/userguide/userguide_insights.md. Source of record for each is that file. -->
...
Three things overlap on purpose and are exempt: a backdrop (`panel`, `fpic`) that sits behind boxes by design; a transparent `button` laid over the comment it makes clickable; and a box that is hidden because code owns its visibility (a tutorial highlight or annotation — see *Never Hide Patchcords or Boxes* in `CLAUDE.md`). *For instance:* the drift-sequencer patches on 2026-09-16 carried 15–17 overlaps each, mostly a label comment and its control 2–7 px into each other, and one `loadmess` running 20 px under the `v8` beside it.

### Give every port room

A box must be at least 24 + 15 × N px wide, where N is the larger of its inlet and outlet counts. Narrower than that, the ports crowd together and cords are hard to tell apart. For instance, `[t b b b]` has 3 outlets, so it needs at least 69 px. Max centres the first and last ports 19 px in from the box's edges (measured by John, 2026-09-16). It covers object boxes (`newobj`) only; message boxes and UI objects keep their own sizes. `convert` enforces it on every object box it writes, widening even a spec `size` that is too small. When editing an existing patch, widen any narrow object box you come across, but do not sweep patches looking for them. `sync` records a narrow saved box's real width, so the next convert widens it, and the verifier's `box-too-narrow` style note reports a narrow `size` when the spec declares the port counts. Rule and scope confirmed by John on 2026-09-16.

### Keep boxes on the grid the patch already uses

When most boxes of a patch sit on a grid — Max's own default is 15 px, and the converter's layout lands on 5 — the few that do not are almost always slips: a box nudged by hand, or pasted and never snapped. They read as misaligned without anyone being able to say why. So **a box follows the grid the rest of the patch is on**. The verifier's `off-grid` check finds that grid for itself: it tries 15, 10 and 5 px, coarsest first, and accepts one when at least 80% of at least five positioned boxes sit on it. A patch laid out freehand fits no grid, and the check says nothing about it.

**Comments are exempt.** A label is deliberately offset from its control to compensate for its text padding, and that offset is an observed-good pattern the rule must not undo (*Modify, Don't Rebuild* in `CLAUDE.md`). Rule confirmed by John on 2026-09-16, when `c2m_layout.py`'s checks were folded into the verifier.
...
**A cord that runs a long way gets the same judgment even through empty space.** Past roughly 600 px end to end, the eye loses which two boxes a cord joins whether or not anything sits under it, and the verifier's `cord-long` check flags it. **When the source is already an `r` / `r~`, do not add another pair — move the receive.** A receive exists so it can sit anywhere, so a long cord out of one means it was placed by its channel name instead of next to what it feeds. *For instance:* in the effect shootouts on 2026-09-16, 169 of the 198 long cords left an `r~` placed in a row with its siblings and ran several hundred pixels to one `selector~` inlet; moving each `r~` above its inlet removes the cord's length with no new objects.
...
- **In Max 9, `route` and `routepass` have one inlet per argument, plus one, and the same number of outlets.** `route a b c` has four inlets: the extra ones change what each outlet matches. Patches saved by Max 7 show a single inlet, so an old patch or an old tutorial is the wrong guide. A double-quoted phrase counts as one argument (`route center "rim tip"` has three inlets). Verified on 2,027 boxes Max 9 saved, 2026-09-16.
...
- **A script that node runs is not a script Max can load — check the file, not just the code.** Max reads a `.js` into a C string, so **a NUL byte truncates the file at that point**: the source stops mid-line, V8 reports a `SyntaxError` on a line that reads perfectly, and *no* function in the file exists — including every one defined above the NUL. The object then answers nothing, and the Console's other line is `no function <whatever you sent>`, which reads like one missing handler rather than a dead script. node's parser accepts a NUL inside a string literal and runs the file happily, and `node --check` passes it, so a green test run says nothing about whether Max can load the artifact. The general rule: **when the target's loader has constraints the language does not, test the artifact against the loader, not only the behaviour against the language.** For a Max script that means a check for a NUL, for any other control character, for a BOM, and for each function Max dispatches to by name — cheap, and it is the only thing standing between a one-character slip and a day of debugging the wrong layer. *For instance:* `butter_comment.js` carried one NUL written into a cache-key separator by the tool that first wrote the file; 221 passing node tests and a clean `node --check` ran over it for a day while every box in its help file drew nothing. `Butter_tools/tests/butter_comment_headless.js` opens with a `source hygiene` section for exactly this. The separator it needed is gone too — a two-level cache keyed by style and then by string needs no delimiter, so no character has to be absent from both halves.
...
- **Max caches abstractions for the session — editing the .maxpat file and reopening the parent does NOT reload the abstraction.** Once an abstraction is loaded into a session, every subsequent instantiation comes from Max's in-memory cache, including freshly-opened parent patches. To pick up edits: close EVERY patch that contains the abstraction (one cached load is enough to keep the stale version pinned), then reopen them — or restart Max. The same applies to an external `.js` file, and **which objects can watch that file is per-object, not per-language**: `js` and `jsui` document an `autowatch` message, `v8` and `v8ui` do not — the V8 objects dropped it, and an `autowatch = 1` line in a script they load is a no-op that claims a behaviour the engine does not have. All four document `compile` ("Recompiles the current file"), which is the cheap way to reload one box without restarting Max. So: `js` / `jsui` with `autowatch = 1` reload themselves; `v8` / `v8ui` keep the copy they loaded first until you send `compile` or restart. (Verified against all four refpages, 2026-09-21, after every `v8ui` in a help file spent a day running a script that had been rewritten a dozen times.) The diagnostic for "I edited the abstraction but my changes aren't showing" is almost always cache-related, not edit-related — the recognition signal is when the patch behaves as if your file change never happened.
...
- **A box attribute is READ with `getboxattr`, not `getattr` — they are different sets.** The bullet above says a box attribute is *written* with `sendbox <attr> <value>`, because the box's attributes and the object's are two separate sets; the reader is the same story. From a `js` / `v8` / `jsui` / `v8ui` script, `box.getattr("textcolor")` asks the OBJECT, which usually has no such attribute and answers nothing, while `box.getboxattr("textcolor")` asks the box and answers. Max's own `Maxobj` declarations carry both, plus `getboxattrnames()` for the box's list. The failure is the quiet kind: the read returns nothing, the code falls back to whatever default it was written with, and the object draws with that default forever. Try `getboxattr` first and `getattr` second, so a name that lives in either set is found. *For instance:* `butter_comment` read `fontname`, `fontsize`, `fontface` and `textcolor` with `getattr`, got nothing for any of them, and drew every comment in fallback black at 12pt Arial.
...
- **A UI object's default size is a convention, not a fact Max publishes — check the box in Max before trusting it.** Nothing in the Max install records the size a new box is created at; it is computed in C per class. Every corpus you might derive it from — a class's own help file, the modal size across all 3,122 shipped C74 patches, the 63 shipped `object-prototypes` — is a collection of boxes somebody already resized, and measured on 2026-09-15 all three disagree with each other and with the converter's own `UI_SIZES` table. `UI_SIZES` is itself partly wrong: it gives `dial` (40, 48) where the shipped prototype and 72 % of Max 9 instances say 40 × 40, and it gives `live.toggle`, `live.numbox` and `live.text` an identical (44, 20) while 96 % of Max 9 `live.toggle` boxes are 15 × 15. So the converter's default is "a size Max shipped for this class", which is a usable box — but when a size matters to a layout, place the object in Max, read its rect, and put that in the spec's `size`. The failure this guards against is the quiet one: a box at the wrong default still opens, still wires, and still works, so nobody looks at it. *For instance:* before the help-file fallback existed, a UI class missing from `UI_SIZES` was sized by the text-width estimate — and since a UI object has no box text, that meant 40 × 22. The repo's own eight shootout patches still carry 268 `attrui` boxes stuck at 40 × 22 from it, too narrow to read the attribute name or its value.
...
A `jsui` / `v8ui` script, or anything else that paints text itself, has no colour of its own to fall back on, and Max patches come in light and dark. So the default is **the patcher's own `textcolor`**, which Max documents as *Comment Text Color* and which follows the theme — not a constant the author picked while looking at one canvas. A hard-coded black is invisible on a dark patch and a hard-coded white on a light one, and neither reports anything: the object draws, the text is there, and nobody can see it. `this.patcher.getattr("textcolor")` is what C74's own help-file scripts ask (`help/resources/helpdetails.js`), with `locked_bgcolor` for the background behind it.

The same holds for a `comment` in the patching view, which is why the rule above says a patching-view comment usually wants no `bgcolor`: **it wants no `textcolor` either.** Max's default is calibrated for whichever canvas the reader is on, and a grey chosen for a light patch is unreadable on a dark one. Set a colour when the colour carries meaning (the semantic palette above), never to "look right" against the canvas in front of you.

Where a box does carry its own `textcolor`, that wins — with one wrinkle worth knowing: an unset box reports opaque black, so a script cannot tell "the author chose black" from "nobody chose anything". Treat opaque black as unset and fall back to the patch's colour; leave a way to ask for black explicitly that is never second-guessed.

For instance: `butter_comment` defaulted to `[0,0,0,1]`, and every instance in its own help file drew black text on Max's dark canvas. The fix was one function — box `textcolor` if it carries a non-black one, else the patcher's, else black — and the captions around them, which had been given a dark grey for a light canvas, had their `textcolor` removed entirely. (2026-09-21.)
...
When packaging a Max abstraction, external, or set of patches for distribution (Package Manager, GitHub release, etc.), follow the canonical Max-package layout. Anything inside this directory shape becomes discoverable to Max via the standard search path, and a properly-formed refpage XML makes the package's objects automatically I/O-resolvable in Claude2Max specs without any per-object override. The converter finds the refpage when the package sits in Max's built-in `C74/packages` or in the user packages folder `~/Documents/Max 9/Packages` (a symlink there counts). It looks in `docs/`, `docs/refpages/`, `docs/refpages1/`, and one domain folder under any of those three — so `docs/jit.fx/`, `docs/refpages/max/`, and `docs/refpages1/common/` are all found, matching the real shapes Jitter Tools, RNBO, and Gen ship. Built-in packages are searched first, so a Cycling '74 page wins a name clash. The same folders are read for `interfaces/obj-qlookup.json` and `interfaces/max.db.json`.
