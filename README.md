...
|---------|-------------|----------------|---------------------|-------------------|------------|-----------|
| **Claude2Max** | Declarative spec → `.maxpat` | Yes | C74 refpage XML (on demand) | 2,943 objects, curated `use_when` | Embedded spec + sync | Built-in (`/c2m-tutorial`, `/c2m-explain`) |
...
- **Curated installed-package library with `use_when`.** 2,943 package objects, each with a hand-written judgment about when to reach for it instead of a long native chain. The LLM consults this *before* composing a chain, not after.
