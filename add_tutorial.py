...
def write_tutorial_js(steps, js_path, all_highlight_ids, annotation_ids):
...
def add_tutorial_to_patch(maxpat, steps, annotation_ids, js_filename, content_bottom_y):
    ...
    ann_y  = ctrl_y + 32             # thispatcher + annotation comments

    # Expand patch height to fit
    ...
    new_boxes = [
        ...
        # loadbang to initialize step 0 on patch open
        {"box": {
        ...
        # thispatcher receives scripting messages from v8
        {"box": {
            "id": "tut-thispatcher", "maxclass": "newobj",
            "numinlets": 1, "numoutlets": 2, "outlettype": ["", ""],
            "patching_rect": [15.0, float(ann_y), 90.0, 22.0],
            "text": "thispatcher",
        }},
    ]
    ...
    for i, (step, ann_id) in enumerate(zip(steps, annotation_ids)):
        ...
        new_boxes.append({"box": {
            ...
            "numinlets": 1, "numoutlets": 0, "outlettype": [],
            "patching_rect": [120.0, float(ann_y), 780.0, 44.0],
            "text": text,
            "hidden": 1,
    ...
    new_lines = [
        ...
        {"patchline": {"source": ["tut-v8",       0], "destination": ["tut-thispatcher", 0]}},
    ]
...
def main():
    ...
    all_highlight_ids = [
        w["box"]["id"]
        for w in boxes
        if w["box"].get("maxclass") not in {"comment", "text.codebox"}
        and w["box"]["id"] not in {"obj-spec-embed"}
        and not w["box"]["id"].startswith("tut-")
    ]
    ...
    write_tutorial_js(steps, js_path, all_highlight_ids, annotation_ids)
