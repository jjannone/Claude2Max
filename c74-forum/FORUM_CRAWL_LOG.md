...
| 2026-05-05b | Pass-4 (local): Gen pp.9-15 + Jitter pp.19-22 + MaxMSP pp.21-24 + Javascript pp.16-19 + RNBO pp.11-12 + Max For Live pp.13-15 + Misc pp.11-12 | 248 | 110 | 14 (across 6 sections) | 72 artifact files saved to `/tmp/forum-pass4/`. State JSON now totals 1177 entries (522 downloaded / 649 skipped-no-artifact / 6 scraped). 5 fresh PROMOTION-CANDIDATE flags applied. WebFetch hit-rate for insight extraction was 15/16 (94%) on the highest-artifact-richness threads. Gen subforum confirmed densest yet again — 7 of the 14 written insights came from Gen alone. **Closing sweep**: all 5 promoted in same pass to `patching/GEN_PATCHING.md` and `patching/MAX_PATCHING.md`. |
| 2026-05-06 | Pass-5 (local): Gen pp.16-25 + Jitter pp.23-32 + MaxMSP pp.25-34 + Javascript pp.20-27 + RNBO pp.13-19 + Max For Live pp.16-22 + Misc pp.13-18 | 548 | 204 | 19 (across 5 sections, Jitter-heavy at 12/19) | 180 artifact files saved to `/tmp/forum-pass5/`. State JSON now totals 1708 entries (726 downloaded / 976 skipped-no-artifact / 6 scraped). 10 fresh PROMOTION-CANDIDATE flags applied — same-pass closing sweep. WebFetch hit-rate 19/20 (95%) on the highest-artifact-richness threads. Pass-5 was Jitter-heavy by sample composition: 12 of 19 written insights are Jitter, reflecting both the subforum's volume and its surface-area for non-obvious GL pipeline gotchas (texture-as-attribute persistence, normalised-coordinate ±2 range, fixed-vs-shader pipeline migration). |

## Resume point

Run from a non-WAF-blocked egress (e.g. local laptop) — the remote Anthropic-cloud env is blocked by Cloudflare. Verify first:

```bash
curl -s -o /dev/null -w "%{http_code}\n" "https://cycling74.com/forums?category=MaxMSP&page=1"
```

If that returns `200`, the next chunk options are (post pass-5):

- **MaxMSP pp.35+** — pass-5 covered pp.25-34
- **Jitter pp.33+** — pass-5 covered pp.23-32
- **RNBO pp.20+** — pass-5 covered pp.13-19
- **Javascript pp.28+** — pass-5 covered pp.20-27
- **Gen pp.26+** — pass-5 covered pp.16-25
- **Misc pp.19+** — pass-5 covered pp.13-18
- **Max For Live pp.23+** — pass-5 covered pp.16-22
