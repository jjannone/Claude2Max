...
- **Presentation** — use `presentation` field for user-facing layouts. Every presented control needs a comment label. **Always design presentation views as functional, logical, and aesthetic UIs** — not just a copy of the patching layout. Apply these principles:
  ...
  - **Use screenshots for layout verification** — after converting, take a screenshot of the patch open in Max (both patching view and presentation view) to verify positioning, alignment, and spacing visually before reporting the work as done. The user has granted permission for screenshots specifically for layout decisions. Fix anything that looks wrong before finishing.
