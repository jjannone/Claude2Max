...
4. **If it is truly hybridizing two objects, import the full vocabulary of the base object and as much of the second object's as possible.** Import **all** attributes/messages from the base object; import as many as possible from the second. Keep the combined behavior as easy and transparent as you can, and **flag any confusions or conflicts (overlapping names, incompatible semantics) for the user to decide** rather than guessing. *For instance:* `butter_keys` (`[v8ui]`, Butter_tools) hybridizes `kslider` (base) with `multislider` (second) — it imports the kslider attributes (`mode`, `offset`, `range`, …) and kslider messages (`int`/`float`, `set`, `chord`, `clear`, `flush`), adds the multislider-flavoured side (`setminmax`, `setval`, `fetch`, `list`, `clearvals`), and selects between the two faces with one new attribute (`@_displaymode`); the multislider messages whose names kslider already owns (`bang`, `int`/`float`, `set`) are sent as `_dump`, `_setall`; multislider's `set <slider> <value>` needs no prefixed form because `select` already does it, so a bare number plays a note on either face, as on kslider. Where the two objects' conventions couldn't both be honored (e.g. right-inlet velocity passthrough), that was surfaced rather than silently dropped.

5. **Mark every addition with a leading underscore, so a reader can tell at a glance what the object added.** An expanded object's own attributes and messages are listed alongside the ones it inherited, in one alphabetical column, and nothing else distinguishes them: a reader who knows the base object cannot otherwise tell which entries are new. One character in front of every addition answers that, and it also keeps a new name from ever colliding with one the base object gains in a later Max. The prefix is `_`: `@_displaymode`, `@_lohikey`, `_dump`, `_setall`. (John, 2026-09-22, replacing the `z_` this rule used to specify; every Butter tool was renamed in the same change.) The same prefix resolves a message collision — when a message the second object contributes has the name of a message the base object already owns, the second object's version takes the prefix and the base object's keeps the bare name in every mode. When the addition is an *option* on an attribute the base object already has, do not add a new attribute for it: extend the existing attribute and put the prefix on the new option, listed after the base object's own values so those keep their names and positions. For instance, butter_keys's `@mode` keeps Monophonic, Polyphonic and Touchscreen and adds `_mono`; a separate `@_mono` attribute would have been wrong.

   **The setter is named `set_` plus the attribute's own name**, which for a prefixed attribute means two underscores: `_mode`'s setter is `set__mode`. That is not decoration — it is what stops it colliding with the base object's `set_mode`, and butter_keys has exactly that pair, kslider's `@mode` beside its own `@_mode`.

   Two consequences worth knowing. An underscore sorts before every lowercase letter, so the additions group at the **top** of the Inspector rather than the bottom; the grouping is the point, and which end it sits at is a preference. And two attributes that differ only by the prefix — `mode` and `_mode` — are one character apart, so a name that would read ambiguously beside its inherited twin deserves a different word, not just the prefix.
...
`patches/waveform-reference.maxpat` carried `_text` and six style attributes as
...
`_text` in the Inspector and save, and reading what Max wrote — necessary
...
The goal is a list a human can scan: `attrui` and the Inspector list an object's attributes **alphabetically by their human-readable `label`, not by the attribute name.** So when you give an attribute a `label` (the `label:` field in `declareattribute`, or any equivalent), the label **must begin with the same word the attribute name begins with.** Then someone who knows the attribute is `@_displaymode` can find it by scanning the list for "_displaymode…"; if the label leads with some other word, the entry is effectively unfindable — the reader has to open and read every line. Lead with the attribute's own leading word, then add clarifying words or a parenthetical.

For instance, all live in `Butter_tools/javascript/butter_keys.js`: `@_displaymode` → `"_displaymode (Notes / Sliders)"`, **not** `"Display Mode (keyboard or sliders)"`; `@offset` → `"Offset (note of the lowest drawn key, any number)"`, **not** `"Low MIDI Key"`; `@range` → `"Range (number of keys)"`, **not** `"Number of Keys"`.

The prefix an expanded object marks its own additions with is *Building Reusable Objects* point 5's business, not this rule's — this rule only says the label opens with whatever the name opens with. Worth knowing which end that puts them, though: a leading `_` sorts **before** every lettered label, so an object's own additions form one block at the top of the list and the base object's attributes follow. (The `z_` prefix this rule's examples used until 2026-09-22 put that block at the bottom instead. Either is a block; neither is scannable unless the labels lead with the name.)
...
**This rule was retired on 2026-09-22 and reinstated the same day, and the round trip is the useful part.** It was retired on the reasoning that the Inspector prints the attribute's name in its own column and `attrui` shows the name on its face, so repeating the name in the label was redundant — 57 labels across `Butter_tools` were rewritten to drop it. John then sent a screenshot of the Inspector with those labels in place: the rows read `z_codepad` / "a code chip's padding…", `z_tablefill` / "a table spans the box…", `z_sendto` / "also send a clicked link…", `z_attrcolor` / "an @attribute inside `c…`", `bubblepoint` / "arrow position along th…", `bgcolor` / "behind the text; alpha…" — **strictly alphabetical by the label**, with the name column along for the ride. (Those rows are quoted as John saw them, so they carry the `z_` prefix the additions had that morning; the prefix is `_` now, and the sorting is what the screenshot is evidence of, not the names.) Showing a name is not the same as sorting by it. An object with thirty attributes then has thirty rows in an order its reader cannot predict, and the name column cannot be scanned because it is not in any order at all.

So the lesson is not "labels repeat names". It is: **when deciding what a label must carry, the question is what the surface SORTS by, not only what it shows.** A column that displays an identifier still leaves it unfindable if the list is ordered by something else. The recognition signal is a list of more than a handful of rows: find out what orders it before deciding what its labels open with.
...
- **Binary / on-off** (`min:0 max:1`) → `style:"onoff"` — renders a **checkbox**, the unambiguous on/off affordance. Any attribute that is conceptually "this state or that one" belongs here even when the two aren't literally "enabled/disabled" — label it so the checked state is clear (e.g. `@_showvalues` → "_showvalues (slider values as lines on the keyboard face)").
...
- **`scans/packages/packages_insights.md`** — Techniques and pitfalls read out of the installed packages themselves (2026-09-16 scan): wrapper conventions, ports that change with arguments, package bugs worth not copying. **`scans/packages/tutorials_insights.md`** does the same for the Delicious Tutorials and Jitter Recipes packages, and **`scans/packages/tutorial_episodes.md`** indexes their episodes by technique, so "which tutorial shows X?" has an answer. `search_pitfalls` covers both insight files.
- **`packages/package_catalog.xlsx`** — The whole installed-package library as one sortable sheet. Filter by category and usefulness to find the few objects worth knowing in an area, and read "Built-in alternative" before reaching for a package object at all.
