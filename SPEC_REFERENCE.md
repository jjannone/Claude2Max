...
| `patcher_extras` | no | unset | Patcher-level state Max writes that is not a box, carried verbatim: `snapshot` (the Snapshots panel's stored snapshots, values keyed by parameter long name) and `parameters` (Max's box-id → parameter-name registry, which Max rebuilds on save). `sync` fills it from the file and removes it when the operator deletes the snapshots in Max; `convert` writes it back. Do not author those two — they exist so a snapshot stored in Max survives the next convert (delay-shootout, 2026-09-09). Two keys *are* authored here: `showontab` and `showrootpatcherontab`, Max's patcher-window tabs. Put `"patcher_extras": {"showontab": 1}` in a `p` box's `patcher` sub-spec and Max shows that subpatcher as a tab of the parent window, titled with the box's name (`p "1 Key sends"`, quotes for spaces); `showrootpatcherontab` on the root decides whether the parent itself gets a tab. This is how every C74 help file is built, and `thispatcher setactivetab <name>` switches tabs. Sub-spec `patcher` dicts are not re-read by `sync`, so a tab flag lives in the spec only. |
...
- **Do not rely on a `v8ui` box's saved attribute keys to restore a declared attribute's value.** The converter writes a declared attribute (`mode`, `range`, a custom `@clicknotes`) as a plain key on the box, the shape the Zendrum help file showed. Max does not apply those keys at load and does not write them back: a `zkeyboard` box saved with `mode: 1` came up in mono mode, and when Live saved the device the keys were gone from the box (2026-09-12/13). The cause is load order — the object's attributes exist only after the script has run, after box keys are applied. So have the patch assert any attribute it depends on by message after load — a `loadmess` into the box, or the engine that drives the board sending `mode 1`, `lohikey 0 127` in its own init — or pass them as `jsarguments`, which Max hands to the script at load. A declared attribute accepts a message of its own name, so the message path is safe.
